...
export default function App() {
  ...
  const getDetailedTileType = (r, c, gridR = rows, gridC = cols) => {
    if (gridR === 1 && gridC === 1) return 'Tấm Đơn (4 Cạnh Phẳng)';

    // Xử lý bản đồ 1 hàng hoặc 1 cột
    if (gridR === 1 || gridC === 1) {
      if (r === 0 && c === 0) return 'Tấm Hẹp Dạng 1 (1 Lồi)';
      if (r === gridR - 1 && c === gridC - 1) return 'Tấm Hẹp Dạng 2 (1 Lõm)';
      return 'Tấm Hẹp Dạng 3 (1 Lồi, 1 Lõm)';
    }

    const isTop = r === 0;
    const isBottom = r === gridR - 1;
    const isLeft = c === 0;
    const isRight = c === gridC - 1;

    // Phân loại các góc (Tận dụng tính chất lật mặt vật liệu để gộp chung file cắt)
    if (isTop && isLeft) return 'Góc Dạng 1 (2 Lồi)'; // Góc Trái Trên
    if (isBottom && isRight) return 'Góc Dạng 2 (2 Lõm)'; // Góc Phải Dưới
    if ((isTop && isRight) || (isBottom && isLeft)) return 'Góc Dạng 3 (1 Lồi, 1 Lõm)'; // Góc Phải Trên & Góc Trái Dưới lật ngược là giống nhau

    // Phân loại các viền (Tận dụng tính chất lật mặt để gộp)
    if (isTop || isLeft) return 'Viền Dạng 1 (2 Lồi, 1 Lõm)'; // Viền Trên & Viền Trái
    if (isBottom || isRight) return 'Viền Dạng 2 (1 Lồi, 2 Lõm)'; // Viền Dưới & Viền Phải

    return 'Trung Tâm (2 Lồi, 2 Lõm)';
  };

  const getDetailedTileColor = (type) => {
    switch (type) {
      case 'Trung Tâm (2 Lồi, 2 Lõm)': return 'fill-emerald-400 bg-emerald-400';
      
      case 'Viền Dạng 1 (2 Lồi, 1 Lõm)': return 'fill-blue-400 bg-blue-400';
      case 'Viền Dạng 2 (1 Lồi, 2 Lõm)': return 'fill-cyan-400 bg-cyan-400';
      
      case 'Góc Dạng 1 (2 Lồi)': return 'fill-orange-400 bg-orange-400';
      case 'Góc Dạng 2 (2 Lõm)': return 'fill-pink-400 bg-pink-400';
      case 'Góc Dạng 3 (1 Lồi, 1 Lõm)': return 'fill-amber-400 bg-amber-400';
      
      case 'Tấm Hẹp Dạng 1 (1 Lồi)': return 'fill-indigo-400 bg-indigo-400';
      case 'Tấm Hẹp Dạng 2 (1 Lõm)': return 'fill-violet-400 bg-violet-400';
      case 'Tấm Hẹp Dạng 3 (1 Lồi, 1 Lõm)': return 'fill-fuchsia-400 bg-fuchsia-400';
      
      case 'Tấm Đơn (4 Cạnh Phẳng)': return 'fill-slate-400 bg-slate-400';
      default: return 'fill-gray-300 bg-gray-300';
    }
  };
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const type = getDetailedTileType(r, c);
      tiles.push({ r, c, type });
  ...
  // Ordered list for display in the sidebar
  const displayOrder = [
    'Trung Tâm (2 Lồi, 2 Lõm)',
    'Viền Dạng 1 (2 Lồi, 1 Lõm)',
    'Viền Dạng 2 (1 Lồi, 2 Lõm)',
    'Góc Dạng 1 (2 Lồi)',
    'Góc Dạng 2 (2 Lõm)',
    'Góc Dạng 3 (1 Lồi, 1 Lõm)',
    'Tấm Hẹp Dạng 1 (1 Lồi)',
    'Tấm Hẹp Dạng 2 (1 Lõm)',
    'Tấm Hẹp Dạng 3 (1 Lồi, 1 Lõm)',
    'Tấm Đơn (4 Cạnh Phẳng)'
  ];
  ...
  const handleLayoutAdvice = async () => {
    ...
    2. Phân loại chi tiết tối ưu file cắt (nhóm các mảnh đối xứng có thể lật mặt để dùng chung): Có bao nhiêu tấm Trung Tâm, Viền Dạng 1/2, Góc Dạng 1/2/3...
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {viewMode === 'ai' ? (
          ...
          <>
                ...
                <div className="flex flex-col items-center">
                  ...
                  >
                    {tiles.map((tile) => {
                      ...
                      const colorClass = getDetailedTileColor(tile.type).split(' ')[0];
                      return (
                        <g
                          key={`${tile.r}-${tile.c}`}
                        ...
                        >
                          <path
                            ...
                            className={`${colorClass} stroke-gray-800 stroke-[1.5px] transition-all duration-200 ${
                  ...
                  {hoveredTile ? (
                    <div className="mt-6 p-4 w-full bg-blue-50 rounded-lg border border-blue-100">
                      <h3 className="font-bold text-lg mb-1 text-blue-800">
                        {hoveredTile.type} (Tọa độ: Hàng {hoveredTile.r + 1}, Cột {hoveredTile.c + 1})
                      </h3>
                      <p className="text-sm text-blue-600">
                        Kích thước bao: 300x300mm. 
                        {hoveredTile.type.includes('Trung Tâm') && ' Liên kết ngàm 4 hướng.'}
                        {hoveredTile.type.includes('Góc') && ' Có 2 cạnh phẳng để áp sát vào góc.'}
                        {hoveredTile.type.includes('Viền') && ' Có 1 cạnh phẳng để áp sát vào viền nhôm.'}
                        <br/>
                        <span className="inline-block mt-3 font-medium text-indigo-700 bg-indigo-100 px-3 py-2 rounded-lg border border-indigo-200 shadow-sm">
                          💡 <strong>Mẹo CNC:</strong> Tấm này dùng chung file cắt với các tấm cùng Dạng. Lúc ráp chỉ cần xoay hoặc lật ngược mặt ván (nếu vật liệu 2 mặt giống nhau) là khớp khít với vị trí đối xứng.
                        </span>
                      </p>
                    </div>
                  ) : (
            ...
            <div className="w-full lg:w-80 flex flex-col gap-4 break-inside-avoid print:mt-6 print:w-full">
              <div className="bg-white p-5 rounded-xl shadow-lg border-t-4 border-gray-800">
                ...
                <ul className="space-y-3">
                  {displayOrder.map(type => {
                    ...
                    // Lấy ra class màu nền (bg-...) để hiển thị ô vuông chú thích
                    const bgClass = getDetailedTileColor(type).split(' ').find(cls => cls.startsWith('bg-')) || 'bg-gray-300';
                    
                    return (
                      <li key={type} className="flex items-center justify-between pb-2 border-b border-gray-50 last:border-0 last:pb-0">
                        <div className="flex items-center">
                          <div className={`w-4 h-4 rounded mr-3 border border-gray-200 shadow-sm ${bgClass}`}></div>
                          <span className="font-medium text-sm text-gray-700">{type}</span>
                        </div>
                        <span className="font-bold text-sm text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{count} Tấm</span>
                      </li>
                    );
                  })}
                </ul>
                <div className="mt-5 pt-4 border-t border-gray-200">
                  <p className="text-sm font-bold flex justify-between items-center">
                    <span className="text-gray-600 uppercase">Tổng cộng:</span>
                    <span className="text-xl text-indigo-700">{totalCount} Tấm</span>
                  </p>
                </div>
              </div>
            </div>
